...
recipesController.get("/", (req: Request, res: Response) => {
  ...
  if (isString(req.query.categoryId)) { // "req.query" : Un objet contenant les paramètres de query string , avant qu'on extrait le contenu number si possible
                                        // GET /recipes?categoryId=3&authorId=4&search=&ingredient=tomate&maxPrepTime=30
                                        //alors :req.query {
                                        //           categoryId="3" --->  toujours une string
                                        //           authorId= "4" ----> toujours une string
                                        //         }
